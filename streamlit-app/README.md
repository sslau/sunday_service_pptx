...
- **左側面板**：切換／新增主日日期、`💾 儲存目前內容`、`📽 製成整場投影片
  （合併＋編譯）`，以及製成後的下載按鈕。
- **編輯內容**：選日期 → 輸入宣召／詩歌／讀經／信息／公佈事項。宣召與讀經可
  只填**出處**（如 `詩篇 34:1-3`、`羅馬書 12:1-8（和合本）`），按
  **`從 fhl.net 聖經網輸入`** 由信望愛聖經網（bible.fhl.net）自動抓取經文，再自動分頁。
- **製成投影片**：
  1. 三個可合併的獨立 pptx：**詩歌敬拜**、**講道信息**、**公佈事項**。
  2. 每一節都可先「個別產生」成獨立 .pptx（在網頁編譯，例如只有歌詞時
     ...
     - 「個別產生」的檔案會**自動儲存在 `data/decks/<date>/`**，之後同一天
       ...
       先前儲存／已產生的各節投影片」開關停用）。
  3. 合併：把該節的 .pptx **上載**（三格里選檔案），該節以檔案為準；留空則用
  ...
  5. 「製成整場投影片」合併固定頁＋三大節 → 下載 `Sunday_Service_<date>.pptx`。
  6. 另可替換節目的背景圖片，或在製成時為「圖片型公佈」附加圖片。

已知取捨：上傳的圖片（例如「圖片型公佈事項」）不回存試算表，重啟後需重新
